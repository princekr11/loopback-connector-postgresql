...
function mixinMigration(PostgreSQL) {
    ...
    this.execute(sql, function(err, fields) {
      if (err) {
        ...
        fields.forEach(function(field) {
          field.type = mapPostgreSQLDatatypes(field.type, field.length, field.time_precision, field.numeric_precision, field.numeric_scale);
        });
        cb(err, fields);
      }
    });
    ...
    if (actualFks) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (existingFks.filter(function(fk) {
          return fk.fkName === keys[i].length <=64 ? keys[i] : keys[i].substring(0,63);
  ...
  function mapPostgreSQLDatatypes(typeName, typeLength, typeTimePrecision, numericPrecision, numericScale) {
    ...
    switch (type) {
      ...
      case 'TIME WITH TIME ZONE':
        return 'TIME ' + strPrecision + 'WITH TIME ZONE';
      case 'NUMERIC':
        return numericPrecision && numericScale ? 'NUMERIC(' + numericPrecision + ','+numericScale + ')' : 'NUMERIC';
      case 'DECIMAL':
        return numericPrecision && numericScale ? 'DECIMAL(' + numericPrecision + ','+numericScale + ')' : 'DECIMAL';
      default:
        return typeName;
    }
  }
  ...
  PostgreSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    ...
    let newIndexNames = {};
    for(let key in m.settings.indexes){
      let tablePrefixedIndexName = self.table(model) + '_' + key;
      newIndexNames[tablePrefixedIndexName] = m.settings.indexes[key];
      newIndexNames[tablePrefixedIndexName].name = tablePrefixedIndexName;
    }
    m.settings.indexes = newIndexNames;
    const indexNames = m.settings.indexes && Object.keys(m.settings.indexes).filter(function(name) {
      return !!m.settings.indexes[name];
    }) || [];
    const sql = [];
    ...
    indexNames.forEach(function(indexName) {
      ...
      if (!found) {
        ...
        const caseInsensitiveUnique = i.options && i.options.caseInsensitiveUnique && i.options.caseInsensitiveUnique === true ? true : false
        // const isUnique = i.options && i.options.unique && i.options.unique === true ? true : false
        const columns = i.keys.map(function(key) {

          if(caseInsensitiveUnique){
            for(let prop in m.properties){
              if(m.properties && m.properties[prop] && m.properties[prop].postgresql && m.properties[prop].postgresql.dataType && m.properties[prop].postgresql.columnName && m.properties[prop].postgresql.columnName == key[0]){
                if(["CHAR","VARCHAR","TEXT"].includes(m.properties[prop].postgresql.dataType)){
                  return ' LOWER(' +self.escapeName(self.column(model, key[0])) + ') ' + (key[1] ? ' ' + key[1] : '')
                }
                else{
    ...
    console.info('Creating unique indexes')
    console.info(sql.join('\n\n'));
    this.query(sql.join(';\n'), cb);
  };

  PostgreSQL.prototype.addIndexesOnForeignKeys = function(model, columnNames, cb) {
    const self = this;
    ...
    columnNames.forEach(columnName=>{
      // The index name used should match the default naming scheme
      // by postgres: <column>_<table>_idx
      const iName = [self.table(model), columnName, 'idx'].join('_').substring(0,63);

      // Don't need to drop the index as this logic is already there in the addIndexes function
      sql.push('DROP INDEX IF EXISTS ' + self.escapeName(iName));
      sql.push('CREATE INDEX ' + self.escapeName(iName) + ' ON ' + self.tableEscaped(model) +
      ' (' + self.escapeName(columnName) + ')');
    });
  ...
  PostgreSQL.prototype.createForeignKeys = function (models, cb) {
    var self = this;
          ...
          if (!err && fields && fields.length) {
            ...
            self.addForeignKeys(model, fkSQL, function(err, result) {
               if (err)done(err)
                ...
                // try to discover foreign keys again and create index where not already present
                self.discoverForeignKeys(self.table(model), {}, function (err, foreignKeys) {
                  if (err) {

                    console.log('Failed to discover "' + self.table(model) +
                    '" foreign keys', err);
                    done(err)
                  }
                  else{
                    const existingIndexes = self._models[model].settings.indexes;
                    let foreignKeyColumns = foreignKeys.map(fk=> fk.fkColumnName);

                    let existingIndexColumns = []
                    for (const idx in existingIndexes){
                      existingIndexColumns.push(existingIndexes[idx].keys[0])
                    }
                    // If there are already existing indexes for the foreign key columns, they should be ignored
                    foreignKeyColumns = foreignKeyColumns.filter(column => {
                      return !existingIndexColumns.includes(column)
                    });
                    self.addIndexesOnForeignKeys(model,foreignKeyColumns,function(err,result){
                      done(err)
                    })
                  }
                })
